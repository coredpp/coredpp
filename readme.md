...
- `DPPAddress` is the ETH address of the wallet that owns the DPP. The address is the `from` field of any transaction that concerns this DPP. This `DPPAddress` provides a way to go from the EDO to its associated DPP.
...
The `DPPAddress` field will contain the address of the *new* passport.
...
As before, `DPPAddress` will now point to the address of the passport that company Z just created.
...
The `parentEDO` would be the EDO of the creation of the first passport. In this case then, the EDO for the creation of the first passport would have two EDOs linked to it, one for the soybean oil and another one for the soybean meal.
